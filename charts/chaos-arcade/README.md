...
chaos-arcade is a tiny web application made with Go 
...
$ helm upgrade -i my-release chaos-arcade/chaos-arcade 
```

The command deploys chaos-arcade on the Kubernetes cluster in the default namespace.
...
The following tables lists the configurable parameters of the chaos-arcade chart and their default values.
...
`image.repository` | `ryneal/chaos-arcade` | Image repository
...
`service.grpcService` | `chaos-arcade` | gPRC service name
...
```console
$ helm install my-release chaos-arcade/chaos-arcade \
...
```console
$ helm install my-release chaos-arcade/chaos-arcade -f values.yaml
```
